    ...
    try {
        ...
        json_build_object(
            ...
            'coordinates', json_build_object(
                'latitude', ST_X(l."coordinates"::geometry),
                'longitude', ST_Y(l."coordinates"::geometry),
            )
...
export const createProperty = async (
    ...
    try {
        ...
        const geocodingResponse = await axios.get(geocodingUrl, {
            headers: {
                "User-Agent": "RealEstateApp (ariefmavlana8@gmail.com)"
            }
        })
        const [latitude, longitude] = geocodingResponse.data[0]?.lat && geocodingResponse.data[1]?.lng
            ? [
                parseFloat(geocodingResponse.data[0].lat),
                parseFloat(geocodingResponse.data[1].lng)
            ]
            : [0, 0]
